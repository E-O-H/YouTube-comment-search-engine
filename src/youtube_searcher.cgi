...
(Tips: you can leave the search box blank and only fill in the filters, e.g. to search for all comments from a user. To filter for multiple users/videos/channels, separate them with space; to match exact names, use double quotes.)
WRITE_HTML
...
if [ -n "$QUERY_STRING" ]; then
   ...
   videoIdQuery=`echo $QUERY_STRING | sed -n 's/^.*videoIdQuery=\([^&]*\).*$/\1/p' | sed 's/%20/ /g'`
   channelTitleQuery=`echo $QUERY_STRING | sed -n 's/^.*channelTitleQuery=\([^&]*\).*$/\1/p' | sed 's/%20/ /g'`
   channelIdQuery=`echo $QUERY_STRING | sed -n 's/^.*channelIdQuery=\([^&]*\).*$/\1/p' | sed 's/%20/ /g'`
